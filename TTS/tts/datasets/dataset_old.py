...
from TTS.tts.utils.text import pad_with_eos_bos, text_to_phoneme_ids, text_to_symbol_ids
...
class TTSDataset(Dataset):
    ...
    def _load_or_generate_phoneme_sequence(
        ...
        if enable_eos_bos:
            phonemes = pad_with_eos_bos(phonemes, character_config=characters)
            phonemes = np.asarray(phonemes, dtype=np.int32)
        return phonemes
            ...
            if self.use_phonemes:
                ...
                text = np.asarray(
                    text_to_symbol_ids(
                        ...
                        character_config=self.characters,
    ...
    def collate_fn(self, batch):
        ...
        if isinstance(batch[0], collections.abc.Mapping):
            ...
            text_lengths = [len(d["char_ids"]) for d in batch]
